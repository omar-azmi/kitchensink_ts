...
import { object_entries, object_fromEntries, object_keys } from "./alias.ts"
import { hmacSha256, hmacSha256Recursive, sha256 } from "./cryptoman.ts"
import { hexStringOfArray, type HexStringReprConfig } from "./stringman.ts"
...
const
	...
	defaultSignedHeadersConfig: Required<S3SignHeadersV4Config<any>> = {
...
export const s3SignHeadersV4 = async <T = Record<string, string>>(
	...
	const
		{ query, headers: additionalHeaders, date, service, payload, method, region }: Required<S3SignHeadersV4Config<T>> = { ...defaultSignedHeadersConfig, ...config },
	...
	const
		...
		canonicalRequest = [method, pathname, query, httpHeadersToString(canonicalHeaders), "", signedHeaders, payloadHash].join("\n")
